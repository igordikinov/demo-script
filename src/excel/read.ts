...
//
// Сигнатура — §3.3:134: функция асинхронная, как writeWorkbook, потому что SheetJS грузится
// через loadXlsx(), и отдаёт ReadResult. Тип snapshots — PmSnapshots: снимки всех карт.
// Загружает их вызывающий (окно загрузки — DN-14, сборка — DN-22), а не разбор: так функция
// остаётся чистой и одинаково работает в браузере и в Node.
//
...
//
// Проверка сигнатуры ZIP `PK\x03\x04` до чтения (§3.3:136) нужна потому, что SheetJS на
// не-ZIP входе (CSV, HTML, случайные байты, пустой буфер) не бросает, а молча отдаёт книгу
// из одного листа — HTML-таблица с «№ шага»/«Шаг» разобралась бы в сценарий. Несовпадение
// сигнатуры (xlsx — всегда ZIP) и исключение чтения одинаково дают E01. Отказ загрузки самой
// SheetJS — не вина файла: промис отклоняется, E01 нет.
...
/**
 * Есть ли на листе хоть одна непустая ячейка (§3.3:137, §3.5:188, DN-1q5). Обход — по
 * ключам объекта листа, а не по `!ref`: диапазон SheetJS берёт из файла как есть, и
 * испорченный `A1:XFD1048576` дал бы миллионы итераций, а число ключей равно числу
 * реальных ячеек. Служебные ключи SheetJS все начинаются с `!` (`!ref`, `!cols`,
 * `!merges`, `!margins`, `!autofilter` и прочие), остальные — адреса ячеек.
 *
 * Именно текст, а не наличие ключа: ячейка остаётся в листе и с пустой строкой, с одними
 * пробелами и от гиперссылки без значения — по §3.3:137 всё это пусто.
 */
function hasAnyText(ws: WorkSheet): boolean {
  return Object.keys(ws).some((key) => {
    if (key.startsWith('!')) return false;
    // Индекс WorkSheet типизирован как any — сужаем явно, как в cellAt.
    const cell: CellObject | undefined = ws[key];
 ...
 *
 * Лист без единой непустой ячейки — W05 «Лист пустой — пропущен» (§3.5:188, решение
 * владельца DN-1q5): забытый «Лист3» виден предупреждением и не роняет загрузку файла.
 * E03 остаётся для листа, где текст есть, а заголовков в строках 1–3 нет (§3.5:179).
 * Пусты все листы — блоков нет, и файл всё равно отклоняется по E02 (§3.5:178).
 *
 * Пустота проверяется по ключам листа (hasAnyText), а не обходом `!ref`. Само наличие
 * `!ref` ничего не решает: SheetJS 0.20.3 теряет `<dimension ref="A1"/>` (для диапазона
 * без двоеточия), поэтому забытый лист из Excel приходит без `!ref`, а лист, где что-то
 * писали и стёрли, — с `!ref` и без ячеек; оба пусты. После guard ячейки есть, значит
 * `!ref` выставлен разбором; `?? 'A1'` — только для типов, отдельной ветки E03 под него
 * нет, чтобы не заводить недостижимый код.
 */
function readBlock(
  X: XlsxModule,
  ...
  const { s, e } = X.utils.decode_range(ws['!ref'] ?? 'A1');
