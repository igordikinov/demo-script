...
/**
 * Сортировка отчёта (§3.5:195): уровень, затем порядок листов, затем строка. Строки уровня
 * книги (`sheet` — `''`) идут раньше листов, затем листы в порядке книги; внутри листа
 * `row: null` (лист целиком) — раньше строк; лист не из `sheets` — после всех (readWorkbook
 * всегда передаёт все листы-блоки, так что это правило действует только при прямом вызове).
 * Сортировка устойчивая: при равенстве сохраняется порядок обнаружения. Вход не меняется.
 *
